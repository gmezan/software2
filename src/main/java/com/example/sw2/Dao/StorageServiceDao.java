...
public class StorageServiceDao {
    ...
    public StorageServiceResponse store(Ventas v, MultipartFile multipartFile) throws IOException {
        if(multipartFile!=null){
            String name = UUID.randomUUID()+ LocalDateTime.now().toString();
            StorageServiceResponse response = new RestTemplate().postForObject(BASE_URL+DOCUMENT_PATH,
                    prepareEntity(multipartFile,name),StorageServiceResponse.class);
            if (response==null){
                return new StorageServiceResponse(){{
                    setStatus("error");
